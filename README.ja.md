...
- **ゴミ箱** — 削除したセッションは7日間ゴミ箱に残り、履歴ごと復元できる。期限を過ぎた分は履歴と添付画像ごと本削除される
...
- **Codexでもコミットできる** — `workspace-write` は `.git` を読み取り専用にするためコミットが通らない。そこでリポジトリのgitディレクトリをサンドボックスの書き込み可能ディレクトリに加える。`--git-common-dir` で解決するのでworktreeでも効く。ワークスペースの外に書く必要があるときは `CLEW_CODEX_WRITABLE_ROOTS` に足せる（`PATH` と同じ区切り）
...
- **リポジトリから作業ディレクトリを選択** — ghqリポジトリ一覧（`GHQ_ROOT`、デフォルト `~/ghq`）から選択
...
- **workerセッション** — セッションがHTTP経由で別のセッションを立てて監視できる。立てたworkerはサイドバーで親セッションの下に入れ子で並ぶ。セッションには環境変数 `CLEW_SESSION_ID` と `CLEW_URL` が渡るので、エージェントは自分自身を親として指定できる
...
入口はWebSocketの `user_message` と同じなので、こうして作ったセッションも普通のセッションになる。ブラウザで開けばいつでも会話に割り込める。`parentSessionId` を付けて作ったセッションは親と同じグループに入り、サイドバーでは親の下に描かれる。`CLEW_SESSION_ID` と `CLEW_URL` はClaude・Codexのどちらのセッションにも渡る。Codexはapp-serverのプロセスを全セッションで共有するため、プロセスのenvではなくスレッドごとの `shell_environment_policy.set` で渡している。
...
5. セッションはブラウザを閉じても生き続け、サイドバーの✕で明示的に削除する。削除はゴミ箱に入れるだけで、サイドバーのゴミ箱アイコンから復元できる。meta・履歴・エージェント側のセッションIDはターン完了ごとにSQLiteへ保存され、サーバー再起動後の最初のメッセージ送信時にエージェントの会話コンテキストごと復元される（DBパスは `CLEW_DB` で変更可）
